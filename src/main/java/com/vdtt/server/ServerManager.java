...
public class ServerManager {
    ...
    public static HashMap<String, SpamCheck> ipSpamMsg = new HashMap<>();
            ...
            users.removeIf(u -> {
                if (u.id == user.id) {
                    if(!timeWaitLogin.containsKey(user.username)) {
                        timeWaitLogin.put(user.username, System.currentTimeMillis() + 10000L);
                    }
                    return true;
                }
                return false;
