...
public class Rong3Sao extends AbsDragon {
    public Rong3Sao(int idEntity, int id, int hp, int level, int x, int y, int kiMax) {
        super(idEntity, id, hp, level, x, y, kiMax);
    }

    @Override
    public void dropItem(Char owner, byte type) {
        ((ZPlanetBlackStar) zone).createNewItemMapStar(this, owner,3);
    }
}
