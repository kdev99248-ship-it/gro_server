...
import com.vdtt.events.TetNguyenDan;
...
public class UseItemTetNguyenDan extends UseItem {

    public UseItemTetNguyenDan() {
        super(TetNguyenDan.class, "Vật phẩm chỉ có thể sử dụng trong sự kiện Tết");
    }
    ...
    protected void use(Char player, Item item) {
        if(item.id == 952) {
            if(player.user.activated == 0) {
                player.getService().openPopupIdGame((byte) 2);
            } else
        ...
        if (item.id == 951) {
            ...
            int idt = Util.nextInt(200);
            if(idt <= 5) {
              isQua = false;   
              player.addZeni(1,true);
            } else if(idt <= 10) {
                isQua = false;
                player.addBallz(10000,true);
            } else if(idt <= 15) {
                int[] ids = new int[] {950,951,952};
                itemId = ids[Util.nextInt(ids.length)];
            } else if(idt <= 50) {
                isQua = false;
                player.addCoin(10000,true);
            } else {
                isQua = false;
                player.getService().serverMessage("Năm mới vui vẻ nhá.");
            }
        }
    ...
    //        newItem.expire = -1;
            ...
            if (item.id == 950) {
                player.addEventPoint(KeyPoint.BANH_CHUNG, 1);
            }
            if (item.id == 951) {
                player.addEventPoint(KeyPoint.BANH_GIAY, 1);
            }
        }
